<template>
  ...
  >
    ...
    <image
      ...
      mode="aspectFill"
    />
    <div class="rc-avatar-name-wrapper" :style="{ backgroundColor: color }">
      <div class="rc-avatar-name-text" :style="{ fontSize: fontSize + 'px' }">
        {{ appellation }}
      </div>
    </div>
  </div>
...
const appellation = computed(() => uni.$UIKitStore.uiStore
    .getAppellation({
...
const handleAvatarClick = () => {
  if (props.gotoUserCard && !isLongPress) {
    ...
    } else if (props.account === uni.$UIKitStore?.userStore?.myUserInfo.accountId) {
        customNavigateTo({
          url: '/pages/user-card/my-detail/index',
        });
      } else {
