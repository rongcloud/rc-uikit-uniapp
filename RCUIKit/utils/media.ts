...
// =================================================================
// Layer 1: 公共 API 接口 (Public API Layer)
// =================================================================
...
/**
 * 读取文件内容为 ArrayBuffer
 * 支持多平台：H5(File对象)、微信小程序、APP原生
 * @param file File对象(H5)或文件路径(微信小程序/APP)
 * @returns Promise<IAsyncRes<ArrayBuffer>>
 */
export const readFileArrayBuffer = async (file: File | string): Promise<IAsyncRes<ArrayBuffer>> => {
  if (isMiniProgram() && typeof file === 'string') {
    return readFileArrayBufferWeixin(file);
  }
  if (isApp() && typeof file === 'string') {
    return readFileArrayBufferNative(file);
  }
  if (file instanceof File) {
    return readFileArrayBufferH5(file);
  }
  return {
    code: ErrorCode.INVALID_PARAMETER_MEDIA_URL,
    msg: '读取文件异常',
 ...
 */
export const readImageBase64H5 = async (file: File): Promise<IAsyncRes<IKitImageInfo>> => new Promise((resolve) => {
...
// =================================================================
// Layer 2: 核心功能模块 (Core Functional Modules)
// =================================================================

// -----------------------------------------------------------------
// Module 2.1: 缩略图与媒体处理 (Thumbnail & Media Processing)
// -----------------------------------------------------------------
...
// -----------------------------------------------------------------
// Module 2.2: 文件内容读取 (File Content Readers)
// -----------------------------------------------------------------
 ...
 * @returns 图片的Base64数据
 */
const readImageBase64Weixin = async (filePath: string): Promise<IAsyncRes<IKitImageInfo>> => {
    const res = await readImageDataWeixin(filePath, 'base64');
 ...
 * @returns 图片的Base64数据
 */
const readImageBase64Native = async (file: any): Promise<IAsyncRes<IKitImageInfo>> => {
  const res = await readFileDataNative(file);
  if (res.code === ErrorCode.SUCCESS) {
    // 删除 base64 中的 data:image/jpeg;base64, 前缀
    const reg = /data:image\/[^;]+;base64,/;
    const base64 = res.data.replace(reg, '');
    return {
      code: ErrorCode.SUCCESS,
      data: {
        thumbnail: base64,
...
const readFileArrayBufferH5 = async (file: File): Promise<IAsyncRes<ArrayBuffer>> => new Promise((resolve) => {
  const reader = new FileReader();
  reader.onload = () => {
    resolve({
      code: ErrorCode.SUCCESS,
...
const readFileArrayBufferNative = async (filePath : string) : Promise<IAsyncRes<ArrayBuffer>> => ({
    code: ErrorCode.INVALID_PARAMETER_MEDIA_URL,
    msg: '当前平台不支持调用 readFileArrayBufferNative 方法',
  });

const readFileArrayBufferWeixin = async (filePath: string): Promise<IAsyncRes<ArrayBuffer>> => {
    const res = await readImageDataWeixin(filePath, 'binary');
    if (res.code === ErrorCode.SUCCESS) {
      return {
        code: ErrorCode.SUCCESS,
...
const readImageDataWeixin = (filePath: string, encoding: 'base64' | 'binary'): Promise<IAsyncRes<any>> => new Promise((resolve) => {
...
const readFileDataNative = async (file: any): Promise<IAsyncRes<any>> => new Promise((resolve) => {
  if (!file) {
    resolve({
      code: ErrorCode.INVALID_PARAMETER_MEDIA_URL,
      msg: '读取文件异常',
...
// =================================================================
// Layer 3: 底层工具 (Low-level Utilities)
// =================================================================
...
const getFile = (path : string): Promise<File | undefined> => new Promise((resolve) => {
  plus.io.resolveLocalFileSystemURL(path, (entry) => {
    ...
    console.error('[RCUIKit-media] getFile error', err);
    resolve(undefined);
  });
});
